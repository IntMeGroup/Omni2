...
def vae_encode_list(vae, x, weight_dtype):
    ...
    # for views in x:
    #     for img in views:
    #         img = vae_encode(vae, img, weight_dtype) #[1,3,512,1024] -> [1,4,64,128]
    #         latents.append(img) 
    latents = [
    torch.cat([vae_encode(vae, img, weight_dtype) for img in idx], dim=0)
    for idx in x
    ]
    
    return latents #latents:len = bs [[6,4,64,64]]
