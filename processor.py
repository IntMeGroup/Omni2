...
class Omni2Processor:
    ...
    def process_multi_modal_prompt(self, text, input_images):
        ...
        if input_images is None or len(input_images) == 0: #len=1
        ...
        #e.g. re.split(pattern, text): '<|user|>\nGenerate an image according to the following instructions\n<img>', '</img> wears <img>', '</img>.<|end|>\n<|assistant|>\n<|diffusion|>'
        for i in range(1, len(prompt_chunks)):
            if prompt_chunks[i][0] == 1:
                prompt_chunks[i] = prompt_chunks[i][1:] #remove start token?

        image_tags = re.findall(pattern, text)  # ['<|image_1|>', '<|image_2|>']
        image_ids = [int(s.split("|")[1].split("_")[-1]) for s in image_tags] #[1, 2]
        ...
        input_images = [input_images[x-1] for x in image_ids] #len=1 len=6 [3, 512, 512]
        ...
        for i in range(len(prompt_chunks)):
            ...
            if i != len(prompt_chunks) -1:
                ...
                views = len(input_images[i])
                size = 0
                for j in range(views):
                    size += (input_images[i][j].size(-2) *  input_images[i][j].size(-1) // 16 // 16)
    ...
    def __call__(self, 
        ...
        for i in range(len(instructions)):
            ...
            if cur_input_images is not None and len(cur_input_images) > 0:
                ...
                cur_input_images = [self.process_360_image(x) for x in cur_input_images] #len=1 len=6 [3,512,512]
            else:
            ...
            if use_input_image_size_as_output: #False
                ...
                input_data.append((mllm_input, neg_mllm_input, img_cfg_mllm_input, [[height, width] for _ in range(6)]))

        if separate_cfg_input: #True
...
class OmniGenCollator:
    ...
    def create_mask(self, attention_mask, num_tokens_for_output_images):
        ...
        padding_images = []
        text_length = attention_mask.size(-1)
        img_length = max(num_tokens_for_output_images) #[2048, 2048]
    ...
    def process_mllm_input(self, mllm_inputs, target_img_size):
        ...
        num_tokens_for_output_images = [
            sum(img_size[0] * img_size[1] // 16 // 16 for img_size in views)
            for views in target_img_size
        ]
        ...
        for x in mllm_inputs:
            ...
            b_inx += 1     
        pixel_values = [[view.unsqueeze(0) for view in sample] for sample in pixel_values]
                       

        
        input_ids = [x['input_ids'] for x in mllm_inputs] #x is a list of {'input_ids', 'pixel_values', 'image_sizes'}
        padded_input_ids, attention_mask, image_sizes = self.pad_input_ids(input_ids, image_sizes) #pad so that every input has same length
