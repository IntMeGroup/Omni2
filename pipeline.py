...
from model import Omni2
from processor import Omni2Processor
from scheduler import Omni2Scheduler
...
class Omni2Pipeline:
    ...
    def from_pretrained(cls, model_name, vae_path: str=None):
        ...
        model = Omni2.from_pretrained(model_name)
        processor = Omni2Processor.from_pretrained(model_name)
        # print('loading vae from:')
        # print(vae_path)
    ...
    def __call__(
        ...
        if max_input_image_size != self.processor.max_image_size:
            self.processor = Omni2Processor(self.processor.text_tokenizer, max_image_size=max_input_image_size)
        self.model.to(dtype)
        if offload_model:
        ...
        if seed is not None:
            ...
            generator = None
        latents = torch.randn(num_prompt, 6, 4, latent_size_h, latent_size_w, device=self.device, generator=generator)
        #latents [1,6,4,64,64]->[1,6,4,64,64]
        latents = torch.cat([latents]*(1+num_cfg), 0).to(dtype)
        #latents [2,4,64,64]->[2,6,,64,64]
        ...
        if separate_cfg_infer:
            for temp_pixel_values in input_data['input_pixel_values']:
                ...
                temp_input_latents = [
                torch.cat([self.vae_encode(img.to(self.device), dtype) for img in idx], dim=0)
                for idx in temp_pixel_values
                ] #len=1 [6,4,64,64]
                input_img_latents.append(temp_input_latents) #convert to list for further process
        else:
        ...
        model_kwargs = dict(input_ids=self.move_to_device(input_data['input_ids']), 
            input_img_latents=input_img_latents,  #len=3 for condition img input [6,4,64,64]
        ...
        # else:
        #     self.model.to(self.device)
        # num_inference_steps = 2 #set inference steps to 2 for debugging
        scheduler = Omni2Scheduler(num_steps=num_inference_steps)
        ...
        # samples [6, 4, 64, 64] 
        # for i in range(6):
        # samples[:,i,:,:,:] = self.vae.decode(samples[:,i,:,:,:]).sample
        samples = torch.squeeze(samples, dim=0) #[6,4,64,64]
