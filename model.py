...
class PatchEmbedMR(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x) #transform to llm hidden state-length
        x = x.flatten(2).transpose(1, 2)  # NCHW -> NLC
        return x

class AdditionalEmbeddings(nn.Module):
    """
    Additional embeddings for 6 views
    """
    def __init__(self, num_views: int, embedding_dim: int):
        super().__init__()
        self.a_e = nn.Embedding(num_views, embedding_dim)  # 定义 embedding 层
        nn.init.constant_(self.a_e.weight, 0) #初始化为全0
    def forward(self, view_indices):
        return self.a_e(view_indices)  # 根据视角索引查找 embedding



class Omni2(nn.Module, PeftAdapterMixin):
    """
    ...
    def __init__(
        ...
        #editted for pos-embed on viewports
        self.view_size = 6
        self.additional_embeddings = AdditionalEmbeddings(self.view_size, hidden_size)
    ...
    def patch_multiple_resolutions(self, latents, padding_latent=None, is_input_images:bool=False):
        ...
        if isinstance(latents, list):
            ...
            for latent, padding in zip(latents, padding_latent): #对每个图片单独处理
                height, width = latent.shape[-2:]
                if is_input_images:
                    latent = self.input_x_embedder(latent)
                else:
                    latent = self.x_embedder(latent) #
                pos_embed = self.cropped_pos_embed(height, width)  #单视角内部的pos-emb   
                latent = latent + pos_embed #pos_embd直接加在latent上
                # additional_emb = self.additional_embeddings(torch.arange(self.view_size, device=latent.device)).unsqueeze(1)  # [6,1,3072]
                # print(additional_emb)
                # latent += additional_emb
                latent = latent.reshape(1, -1, latent.shape[-1])
                if padding is not None:
                    latent = torch.cat([latent, padding], dim=-2)
                patched_latents.append(latent)

                num_tokens.append(pos_embed.size(1)*self.view_size)
                shapes.append([height, width]*self.view_size)
            if not return_list: #return_list = true for infer_with_cfg #return_list = false for training
    ...
    def forward(self, x, timestep, input_ids, input_img_latents, input_image_sizes, attention_mask, position_ids, padding_latent=None, past_key_values=None, return_past_key_values=True, offload_model:bool=False):
        ...
        #num_toekns represent tokens for output image len=bs [num_tokens_for_single_output_image]
        ...
        if input_ids is not None: #None for now
        ...
        if input_is_list:
            image_embedding = output[:, -max(num_tokens):] #[bs, 6144, 3072]
            time_emb = self.t_embedder(timestep, dtype=x.dtype)  #[2, 3072]
            x = self.final_layer(image_embedding, time_emb) #[bs,6144,16]
            latents = []
            
            for i in range(x.size(0)): # for i in bs
                num_tokens_per_img = num_tokens[i]//6
                latent = x[i:i+1, :num_tokens[i]] #[6144, 16]
                    # latent_0 = self.unpatchify(latent[1, 0:1024,:],shapes[i][0], shapes[i][1])
                    # latent_1 = self.unpatchify(latent[1, 1024:2048,:],shapes[i][0], shapes[i][1])
                    # latent_2 = self.unpatchify(latent[1, 2048:3072,:],shapes[i][0], shapes[i][1])
                    # latent_3 = self.unpatchify(latent[1, 3072:4096,:],shapes[i][0], shapes[i][1])
                    # latent_4 = self.unpatchify(latent[1, 4096:5120,:],shapes[i][0], shapes[i][1])
                    # latent_5 = self.unpatchify(latent[1, 5120:,:],shapes[i][0], shapes[i][1])
                latents_view = [
                self.unpatchify(latent[:, 0:num_tokens_per_img, :], shapes[i][0], shapes[i][1]),
                self.unpatchify(latent[:, num_tokens_per_img:num_tokens_per_img*2, :], shapes[i][0], shapes[i][1]),
                self.unpatchify(latent[:, num_tokens_per_img*2:num_tokens_per_img*3, :], shapes[i][0], shapes[i][1]),
                self.unpatchify(latent[:, num_tokens_per_img*3:num_tokens_per_img*4, :], shapes[i][0], shapes[i][1]),
                self.unpatchify(latent[:, num_tokens_per_img*4:num_tokens_per_img*5, :], shapes[i][0], shapes[i][1]),
                self.unpatchify(latent[:, num_tokens_per_img*5:, :], shapes[i][0], shapes[i][1])
                ]
                latents_view = torch.cat(latents_view, dim=0)
                latents.append(latents_view) #len=bs [6,4,64,64]
        else:
    ...
    def forward_with_separate_cfg(self, x, timestep, input_ids, input_img_latents, input_image_sizes, attention_mask, position_ids, cfg_scale, use_img_cfg, img_cfg_scale, past_key_values, use_kv_cache, offload_model):
        ...
        for i in range(len(input_ids)): #len = 2 for with_seperate_cfg
            temp_out, temp_pask_key_values = self.forward([torch.squeeze(x[i], dim=0)], timestep[i], input_ids[i], input_img_latents[i], input_image_sizes[i], attention_mask[i], position_ids[i], past_key_values=past_key_values[i], return_past_key_values=True, offload_model=offload_model)
            model_out.append(temp_out)
            pask_key_values.append(temp_pask_key_values)

        if len(model_out) == 3:
            cond, uncond, img_cond = model_out
            cond = torch.unsqueeze(torch.cat(cond, dim=0), dim=0)
            uncond = torch.unsqueeze(torch.cat(uncond, dim=0), dim=0)
            img_cond = torch.unsqueeze(torch.cat(img_cond, dim=0), dim=0)

            cond = uncond + img_cfg_scale * (img_cond - uncond) + cfg_scale * (cond - img_cond)
            model_out = [cond, cond, cond]
        elif len(model_out) == 2: #condition_out & uncondition_out
